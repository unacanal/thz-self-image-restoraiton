...
def test_measure_BRISQUE(imgPath):
    ...
    # load model 
    model = svmutil.svm_load_model("BRISQUE/Python/libsvm/python/allmodel")
...
if __name__ == '__main__':
    ...
    niqes = {}
    brisques = {}
    basedir = f'/root/why/thz-self-image-restoraiton/results_main/o2o_latest_ZSSR/{THZ}/'
    img_list = sorted(glob.glob(basedir+'*.png'))
    print("Image 개수:", len(img_list))
    for path in img_list:
        ...
        sharps[path] = round(sharpness, 2)

        # Contrast
        cont = img.std()
        conts[path] = round(cont, 2)

        # NIQE
        niqe_score = niqe.niqe(np.array(img))
        niqes[path] = round(niqe_score, 2)

        # BRISQUE
        brisque_score = test_measure_BRISQUE(path)
        brisques[path] = round(brisque_score, 2)
    ...
    sorted_cont = sorted(conts.items(), key=lambda item: item[1], reverse=True)
    sorted_sharp = sorted(sharps.items(), key=lambda item: item[1], reverse=True)
    sorted_niqe = sorted(niqes.items(), key=lambda item: item[1], reverse=True)
    sorted_brisque = sorted(brisques.items(), key=lambda item: item[1], reverse=True)

    print('----- Contrast Top 20 -----')
    for num, i in enumerate(sorted_cont[:20]):
        print(basename(i[0]), i[1], sharps[i[0]])
        save_dir = f'/root/why/thz_self-image_resotration/IQA/o2o_{THZ}THz/cont_best/'
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f'top{num}_{basename(i[0])}')
        shutil.copy(i[0], save_path)

    print('----- Sharpness Top 20 -----')
    for num, i in enumerate(sorted_sharp[:20]):
        print(basename(i[0]), conts[i[0]], i[1])
        save_dir = f'/root/why/thz_self-image_resotration/IQA/o2o_{THZ}THz/sharp_best/'
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f'top{num}_{basename(i[0])}')
        shutil.copy(i[0], save_path)

    print('----- NIQE Top 20 -----')
    for num, i in enumerate(sorted_niqe[:20]):
        print(basename(i[0]), niqes[i[0]], i[1])
        save_dir = f'/root/why/thz_self-image_resotration/IQA/o2o_{THZ}THz/niqe_best/'
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f'top{num}_{basename(i[0])}')
        shutil.copy(i[0], save_path)
    
    print('----- BRISQUE Top 20 -----')
    for num, i in enumerate(sorted_niqe[:20]):
        print(basename(i[0]), niqes[i[0]], i[1])
        save_dir = f'/root/why/thz_self-image_resotration/IQA/o2o_{THZ}THz/brisque_best/'
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, f'top{num}_{basename(i[0])}')
        shutil.copy(i[0], save_path)
    
    # Save as CSV file
    def split_keys(dictionary):
        return {basename(k).split('(')[-1].split(')')[0]: v for k, v in dictionary.items()}
    
    sharps = split_keys(sharps)
    conts = split_keys(conts)
    niqes = split_keys(niqes)
    brisques = split_keys(brisques)
    
    df = pd.DataFrame([sharps, conts, niqes, brisques]).T
    # df = df.rename(columns={0:'Sharpness', 1:'Contrast', 2}, {})
    df.columns = ['Sharpness', 'Contrast', 'NIQE', 'BRISQUE']
    df.to_csv(f'o2o_iqa_{THZ}THz.csv')
